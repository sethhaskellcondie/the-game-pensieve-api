...
/**
 * Holds the resolved owner id and {@link Role} for the duration of a request on the current thread. Populated by
 * {@link TenantTransactionFilter} and cleared in its {@code finally}. The authoritative tenant boundary is the
 * database (the {@code app.current_owner} session variable + Row-Level Security); this holder is a convenience
 * for application code that needs the current owner/role without re-deriving it from the {@code SecurityContext}
 * — and, crucially, the role is captured here <em>before</em> the request drops to {@code app_rls}, which cannot
 * read {@code users}.
 *
