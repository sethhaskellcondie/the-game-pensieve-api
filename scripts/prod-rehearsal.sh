...
#   KC_ADMIN_UI_PASSWORD    plaintext of KC_ADMIN_UI_PASSWORD_HASH; enables the positive basic-auth
#                           check. A generated env file sets it for you.
#
...
if [[ ! -f "$ENV_FILE" ]]; then
    ...
    ui_password="rehearsal-$(openssl rand -hex 8)"
    # `caddy hash-password` in the same image the stack runs, so the bcrypt cost matches production.
...
KC_ADMIN_UI_USER=rehearsal
KC_ADMIN_UI_PASSWORD_HASH='$ui_hash'
# Not read by compose — this script uses it for the positive basic-auth check.
...
for required in APP_DOMAIN MCP_DOMAIN AUTH_DOMAIN SESSION_SECRET OIDC_CLIENT_SECRET \
                POSTGRES_PASSWORD KC_DB_PASSWORD KC_ADMIN_USER KC_ADMIN_PASSWORD \
                KC_ADMIN_UI_USER KC_ADMIN_UI_PASSWORD_HASH; do
...
# network also sidesteps the Caddy basic-auth gate that fronts /admin on the public hostname.
